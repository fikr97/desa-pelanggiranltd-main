...
USING (
  ...
  (
    ...
    (
      ...
      (visibilitas_dusun = 'tertentu' AND (SELECT dusun FROM public.profiles WHERE user_id = auth.uid()) = ANY(form_tugas.dusun_terpilih))
    )
  )
);
    ...
    (
      ...
      (
        ...
        (SELECT dusun FROM public.profiles WHERE user_id = auth.uid()) = ANY(
          (SELECT dusun_terpilih FROM form_tugas WHERE id = form_tugas_data.form_tugas_id)::text[]
        )
      )
    )
    AND
