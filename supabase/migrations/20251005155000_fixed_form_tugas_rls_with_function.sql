-- KOREKSI FINAL: Perbaikan untuk Form Tugas dengan Visibilitas Dusun

-- Hapus semua kebijakan yang ada terlebih dahulu untuk membersihkan
...
-- Fungsi bantu untuk memeriksa apakah dusun ada dalam array
CREATE OR REPLACE FUNCTION is_dusun_in_list(user_dusun TEXT, dusun_list TEXT[])
RETURNS BOOLEAN AS $$
BEGIN
  RETURN user_dusun = ANY(dusun_list);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Kebijakan untuk form_tugas (hanya SELECT berdasarkan visibilitas, CRUD hanya untuk admin)
...
USING (
  ...
  (
    ...
    (
      ...
      (visibilitas_dusun = 'tertentu' AND is_dusun_in_list((SELECT dusun FROM public.profiles WHERE user_id = auth.uid()), dusun_terpilih))
    )
  )
);
...
-- Kebijakan untuk form_tugas_data dengan pendekatan yang diperbaiki
...
-- Beri izin eksekusi fungsi ke authenticated users
GRANT EXECUTE ON FUNCTION is_dusun_in_list TO authenticated;
