-- MIGRASI PERBAIKAN: Memperbaiki fungsi update_form_data_with_penduduk_check agar tidak menghapus data saat pengeditan

-- Hapus fungsi lama
DROP FUNCTION IF EXISTS update_form_data_with_penduduk_check(UUID, UUID, UUID, JSONB);

-- Buat ulang fungsi dengan pendekatan yang lebih aman
...
BEGIN
    ...
    -- Periksa apakah record yang akan diupdate benar-benar ada dan milik pengguna saat ini (jika bukan admin)
    ...
    -- Update data - hanya lakukan update field-field yang tidak null
    UPDATE form_tugas_data 
    SET 
        penduduk_id = COALESCE(p_penduduk_id, penduduk_id),
        data_custom = COALESCE(p_data_custom, data_custom),
        updated_at = NOW()
