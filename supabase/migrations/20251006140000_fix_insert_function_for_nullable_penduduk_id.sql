-- MIGRASI PERBAIKAN TAMBAHAN: Memperbarui fungsi insert_form_data_for_all_dusun 
-- agar mendukung data tanpa penduduk terkait

-- Hapus fungsi lama
DROP FUNCTION IF EXISTS insert_form_data_for_all_dusun(UUID, UUID, JSONB);

-- Buat fungsi baru dengan pengecualian untuk data tanpa penduduk
...
BEGIN
    ...
    RAISE LOG 'Insert check - user_role: %, user_dusun: %, penduduk_dusun: %, form_visibility: %, penduduk_id: %', user_role, user_dusun, penduduk_dusun, form_visibility, p_penduduk_id;
    ...
    ELSIF user_role = 'kadus' THEN
        IF form_visibility = 'semua_data' THEN
            -- Dalam mode 'semua_data', kadus bisa menambah data apa pun, termasuk yang tidak terkait dengan penduduk
            allowed := TRUE;
        ...
        ELSIF p_penduduk_id IS NULL THEN
            ...
            allowed := TRUE;
            RAISE LOG 'Kadus insert access granted for non-resident entry in restricted mode';
