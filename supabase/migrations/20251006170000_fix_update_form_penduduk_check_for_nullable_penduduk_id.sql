-- MIGRASI PERBAIKAN: Memperbaiki fungsi update_form_data_with_penduduk_check 
-- agar mendukung data tanpa penduduk terkait

-- Hapus fungsi lama
DROP FUNCTION IF EXISTS update_form_data_with_penduduk_check(UUID, UUID, UUID, JSONB);

-- Buat ulang fungsi dengan pengecualian untuk data tanpa penduduk
...
BEGIN
    ...
    RAISE LOG 'Update check - user_role: %, user_dusun: %, penduduk_dusun: %, form_visibility: %, penduduk_id: %', user_role, user_dusun, penduduk_dusun, form_visibility, p_penduduk_id;
    ...
    ELSIF user_role = 'kadus' AND p_penduduk_id IS NULL THEN
        -- Jika tidak ada penduduk yang dipilih, izinkan dalam mode non-semua_data
        allowed := TRUE;
        RAISE LOG 'Kadus access granted for non-resident entry in restricted mode';
